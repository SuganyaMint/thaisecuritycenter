...
function EditItemComponent(props) {
  ...
  const onFinish = async (values) => {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "ต้องการแก้ไข Item ใน Hotpack หรือไม่?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        const res = await API.post(ApiRouter.EditAllItem, data);
        if (res.data.status === true) {
          Swal.fire({
            title: "Success!",
            text: "แก้ไข Item ใน Hotpack สำเร็จ",
            icon: "success",
          ...
          Swal.fire({
            title: "Error!",
            text: "เกิดข้อผิดพลาด ไม่สามารถแก้ไข Item ใน Hotpack ได้ โปรดลองใหม่อีกครั้ง",
            icon: "error",
            showConfirmButton: false,
            timer: 1500,
          });
        ...
        Swal.fire({
          title: "Cancel!",
          text: "ยกเลิกการแกห้ไข Item ใน Hotpack",
          icon: "error",
          showConfirmButton: false,
          timer: 1500,
        });
