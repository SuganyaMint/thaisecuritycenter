...
import CreateLogoModel from "../../components/ModelComponent/CreateLogoModel";
...
function LogoPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await API.get(ApiRouter.Logo);
  ...
  const handleDelete = async (id) => {
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const res = await API.delete(ApiRouter.Logo + id);
          if (res.data.status === true) {
            Swal.fire({
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <Title level={2}>Logo Management</Title>
          <div
            style={{
              display: "flex",
          ...
          >
            <CreateLogoModel setIsSubmit={setIsSubmit} />
          </div>
          <Divider />
          <Paragraph>
